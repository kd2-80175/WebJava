...
public int save(Users u) throws Exception {
	...
	try(PreparedStatement stmt = con.prepareStatement(sql)) {
		...
		stmt.setDate(5, DateTimeUtil.utilToSqlDate(u.getBirth()));
...
public int update(Users u) throws Exception {
	...
	try(PreparedStatement stmt = con.prepareStatement(sql)) {
		...
		stmt.setString(2, u.getLastname());
		stmt.setString(3, u.getEmail());
		stmt.setDate(4, DateTimeUtil.utilToSqlDate(u.getBirth()));
	...
	try(PreparedStatement stmt = con.prepareStatement(sql)) {
		...
		try(ResultSet rs = stmt.executeQuery()) {
			while(rs.next()) {
				...
				Date uDate = DateTimeUtil.sqlToUtilDate(rs.getDate("birth"));
